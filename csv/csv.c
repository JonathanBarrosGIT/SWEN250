...
int get_field(f_string field) {
    ...
    for (i = 0; !is_end_of_field(character = getchar()); i++)
    {
        field[i] = character;
    }
    field[i] = '\0';
...
csv_line get_line() {
    ...
    int character = get_field(line.field[i++]);

    // When the first comma is found, we can assume there are already 2 fields
    if(character == ',')
    {
    	line.nfields += 2;
    }
    ...
    while(character != '\n' && character != EOF)
    {
    	character = get_field(line.field[i++]);
...
void print_csv(csv_line header, csv_line data) {
	int i ;
    int limit = min(header.nfields, data.nfields) ;
